...
const STATS_ROUTE = "/stats/opponent-player-game";
const TABLE_SELECTOR = "#stats-results-table";
const WRAPPER_SELECTOR = "#stats-results-table_wrapper";

async function goToStatsTable(page) {
    const response = await page.goto(STATS_ROUTE);
    ...
    try {
        return await page.evaluate(
            (wrapperSelector) => {
                ...
                // Build predictable rows when the backend returns an empty dataset.
                ...
                for (let rowIdx = 0; rowIdx < 2; rowIdx += 1) {
                    ...
                    for (let colIdx = 0; colIdx < headerCount; colIdx += 1) {
                        ...
                        if (colIdx === 0) {
                            td.textContent = `Pinned Player ${rowIdx + 1}`;
                        } else if (colIdx === 1) {
...
test.describe("Stats DataTable sticky first column", () => {
    test("keeps first column pinned and opaque while horizontal scrolling", async ({
        page,
    }) => {
        if (!(await goToStatsTable(page))) {
            return;
        }
        ...
        const before = await page.evaluate((wrapperSelector) => {
            ...
            return {
                ...
                secondCellX: secondCell.getBoundingClientRect().x,
        ...
        // Sticky column should not move while other columns move left.
        expect(Math.abs(after.firstCellX - before.firstCellX)).toBeLessThan(2);
        expect(after.secondCellX).toBeLessThan(before.secondCellX - 20);
