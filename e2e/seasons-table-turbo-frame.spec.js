...
test.describe('Seasons Table - Turbo Frame Navigation', () => {
    ...
    test('should have turbo-frame element for seasons table', async ({ page }) => {
    ...
    test('should handle filter toggle links', async ({ page }) => {
        ...
        const filterLink = page.locator('a[data-turbo-frame="seasons-table-frame"]').first();
    ...
    test('should handle frame navigation to season details', async ({ page }) => {
        await page.waitForLoadState('networkidle');
...
test.describe('Seasons Table - Turbo Frame Filtering', () => {
    test('should update frame content when filter applied', async ({ page }) => {
        await page.goto('/seasons');
        await page.waitForLoadState('networkidle');

        const turboFrame = page.locator('turbo-frame#seasons-table-frame');
        const initialContent = await turboFrame.textContent();
    ...
    test('should preserve frame isolation during filter', async ({ page }) => {
        ...
        const turboFrameBefore = page.locator('turbo-frame#seasons-table-frame');
        ...
        if ((await filterLink.count()) > 0) {
            ...
            const turboFrameAfter = page.locator('turbo-frame#seasons-table-frame');
