...
test("returns null when no table present", async () => {
    ...
    const mod = await import("../../legacy/modules/seasons-init.js");
...
test("initializes DataTable and shows placeholder when SearchBuilder missing", async () => {
    ...
    window.$ = function (sel) {
        const isTableSel =
            sel === "#seasons-table" || sel === table || sel === table.id;
        if (isTableSel) {
            return {
                length: 1,
        ...
        if (isTableSel) {
            return {
                ...
                DataTable: function (options) {
                    ...
                    table.querySelectorAll("thead th")[0].textContent =
                        "Changed A";
                    const apiObj = {
                        columns: { adjust: () => ({ draw: () => {} }) },
                    };
...
test("applies columnDefs from columnLabels and preserves custom defs", async () => {
  ...
  `;
    ...
    expect(capturedOptions.columnDefs.some((def) => def.targets === 0)).toBe(
        true,
    );
    expect(capturedOptions.columnDefs.some((def) => def.targets === 99)).toBe(
        true,
    );
});
        ...
        if (isTableSel) {
            return {
                ...
                DataTable: function (options) {
                    table.querySelectorAll("thead th")[0].textContent =
                        "Changed";
                    scrollHead.querySelectorAll("th")[0].textContent =
                        "Changed";
                    const apiObj = {
                        columns: { adjust: () => ({ draw: () => {} }) },
                    };
