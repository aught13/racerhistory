...
test("buildFieldControl creates number input with min/max", async () => {
    ...
    const { buildFieldControl } =
        await import("../../legacy/games_sport_dynamic.js");
    const field = {
        field_name: "goals",
        field_type: "number",
        min: 0,
...
test("groupFields groups by field_group", async () => {
    document.body.innerHTML =
        '<select id="team-season-select"></select><div id="sport-specific-section"></div>';
    const { groupFields } = await import("../../legacy/games_sport_dynamic.js");
    const fields = [
...
test("renderEav chunks fields into rows of up to 4", async () => {
    document.body.innerHTML =
        '<select id="team-season-select"></select><div id="sport-specific-section"></div>';
    const { renderEav } = await import("../../legacy/games_sport_dynamic.js");
    const template = [];
...
test("fetchMeta uses HTML path when available and updates sport name", async () => {
  ...
  `;
    ...
    const { fetchMeta } = await import("../../legacy/games_sport_dynamic.js");
    await fetchMeta("123");
    const section = document.getElementById("sport-specific-section");
...
describe("games_sport_dynamic helpers", () => {
    ...
    test("fetchMeta sets sport name and renders eavTemplate on success", async () => {
        const modImport = await import("../../legacy/games_sport_dynamic.js");
