...
export function initRosterMultiAdd() {
    ...
    // Initialise person search on all initial rows (supports bulk edit with
    // multiple pre-populated rows as well as the single empty row in add mode)
...
function initPersonSearch(row, searchUrl) {
    ...
    // Bind clear button if already present (e.g. pre-populated in bulk edit)
...
function addRow(container, searchUrl) {
    ...
    newRow.querySelectorAll("input").forEach((input) => {
        // Remove hidden ID field from cloned rows (new entries have no record ID)
        if (input.name && input.name.match(/rows\[\d+\]\[id\]$/)) {
            input.remove();
            return;
        }
        input.value = "";
