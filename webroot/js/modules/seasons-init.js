...
export default function initSeasons(opts = {}) {
    ...
    function destroyExisting() {
        ...
        try {
            if (sbInstance) {
                ...
                else if (sbInstance.dom && sbInstance.dom.container) window.$(sbInstance.dom.container).remove();
                else if (typeof sbInstance.container === 'function') window.$(sbInstance.container()).remove();
            }
        } catch { /* no-op */ }
        sbInstance = null;
    ...
    function trySetupSearchBuilder(dtApi) {
        try {
            ...
            if (sbInstance) {
                try { window.$(panelSelector).empty(); } catch { /* no-op */ }
                try {
            ...
            try {
                sbInstance = new window.$.fn.dataTable.SearchBuilder(dtApi, { depthLimit: 2, columns: sbColumns });
                let containerEl = null;
