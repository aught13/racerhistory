...
describe('person-image.js edge cases', () => {
    ...
    test('initPersonImageSelector with missing elements does not throw', () => {
        ...
        expect(() =>
            personImage.initPersonImageSelector({ selectBtnId: 'x', fieldId: 'y', previewId: 'z' })
        ).not.toThrow();
    });
    ...
    test('uploadFile with network error rejects', async () => {
        ...
        await expect(personImage.uploadFile(new File([blob], 'bad.png'))).rejects.toThrow('fail');
    });

    test('uploadFile with invalid server response rejects', async () => {
        global.fetch = jest.fn(() =>
            Promise.resolve({ text: () => Promise.resolve('<html>fail</html>') })
        );
        const blob = new Blob(['x'], { type: 'image/png' });
