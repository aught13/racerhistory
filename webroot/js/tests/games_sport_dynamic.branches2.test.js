describe('games_sport_dynamic additional branches', () => {
    ...
    test('buildFieldControl falls back to text input for non-number types', () => {
        const field = { field_name: 'note', display_label: 'Note', field_type: 'text' };
    ...
    test('groupFields groups by field_group and renderEav creates cards', () => {
        ...
        expect(Object.keys(groups).sort()).toEqual(['alpha', 'beta', 'general'].sort());
        ...
        const headings = Array.from(
            document.querySelectorAll('#sport-specific-section .card-header h6')
        ).map((h) => h.textContent);
        expect(headings.some((h) => /Alpha|General|Beta/i.test(h))).toBeTruthy();
    });
});
