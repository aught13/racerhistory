...
test('onFileSelected initializes Cropper and displays UI', () => {
  ...
  const fileInput = document.createElement('input'); fileInput.type = 'file'; fileInput.id = `${modalId}-file-input`;
  const cropContainer = document.createElement('div'); cropContainer.id = `${modalId}-crop-container`;
  const cropImage = document.createElement('img'); cropImage.id = `${modalId}-crop-image`;
  const cropPreview = document.createElement('div'); cropPreview.id = `${modalId}-crop-preview`;
  const noPreview = document.createElement('div'); noPreview.id = `${modalId}-no-preview`;
  const cropControls = document.createElement('div'); cropControls.id = `${modalId}-crop-controls`;
  const uploadBtn = document.createElement('button'); uploadBtn.id = `${modalId}-upload-btn`;
...
test('onUploadImage handles crop path and upload failure/success', async () => {
  ...
  document.body.appendChild(modal);
  document.body.appendChild(uploadBtn);
  const target = document.createElement('input'); target.id = 'targetCrop2'; document.body.appendChild(target);

  window.imageSelectorConfig = { [modalId]: { targetFieldId: 'targetCrop2' } };

  // create a fake cropper returning a canvas with toBlob
  const toBlobFn = jest.fn((cb) => cb(new Blob(['b'], { type: 'image/jpeg' })));
  ...
  // mock csrf meta
  const meta = document.createElement('meta'); meta.name = 'csrfToken'; meta.content = 'token'; document.head.appendChild(meta);
  ...
  global.fetch = jest.fn().mockResolvedValue({ ok: true, json: async () => ({ success: true, image: { id: 1234 } }) });
  await inst.onUploadImage();
