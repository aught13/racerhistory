...
test("buildExtraFields handles JSON string, numeric string, array and bulkAction", () => {
    const mod = require("../admin.js");
    const f1 = mod.__internals.buildExtraFields({
        ids: '["a","b"]',
        idsName: "ids[]",
    });
    expect(f1.length).toBe(2);
    const f2 = mod.__internals.buildExtraFields({
        ids: " 42 ",
        idsName: "ids[]",
    });
    expect(f2.length).toBe(1);
    expect(f2[0].value).toBe("42");
    const f3 = mod.__internals.buildExtraFields({
        ids: ["x", "", null],
        idsName: "ids[]",
        bulkAction: "delete",
    });
...
test("submitTempForm appends form with tokens and extra fields and logs submission", () => {
    ...
    mod.__internals.submitTempForm("/do-delete", tokens, [
        { name: "x", value: "y" },
    ]);
