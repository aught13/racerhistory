...
describe('crop-selector branch coverage', () => {
  ...
  test('setAspectRatio snaps crop to ratio', () => {
    ...
    expect(Math.abs(selector.cropBox.width - selector.cropBox.height)).toBeLessThanOrEqual(1);
  });
  ...
  test('onMouseDown starts resizing when handle hit', () => {
    ...
    selector.onMouseDown({ clientX: 10, clientY: 10, preventDefault: jest.fn() });
  ...
  test('onMouseMove resizes with aspect ratio', () => {
    ...
    selector.onMouseMove({ clientX: 150, clientY: 150, preventDefault: jest.fn() });
  ...
  test('onMouseMove resizes without aspect ratio for top handle', () => {
    ...
    selector.onMouseMove({ clientX: 60, clientY: 5, preventDefault: jest.fn() });
  ...
  test('onMouseMove resizes without aspect ratio for right handle', () => {
    ...
    selector.onMouseMove({ clientX: 140, clientY: 60, preventDefault: jest.fn() });
  ...
  test('onMouseMove resizes with aspect ratio for left and bottom handles', () => {
    ...
    selector.onMouseMove({ clientX: 10, clientY: 50, preventDefault: jest.fn() });
    expect(Math.abs(selector.cropBox.width - selector.cropBox.height * 2)).toBeLessThanOrEqual(1);

    selector.resizeHandle = 'b';
    selector.onMouseMove({ clientX: 60, clientY: 120, preventDefault: jest.fn() });
    expect(Math.abs(selector.cropBox.width - selector.cropBox.height * 2)).toBeLessThanOrEqual(1);
  });
  ...
  test('onMouseDown inside crop box starts dragging', () => {
    ...
    selector.onMouseDown({ clientX: 40, clientY: 40, preventDefault: jest.fn() });
  ...
  test('onMouseMove updates cursor based on handle and position', () => {
    ...
    selector.onMouseMove({ clientX: 10, clientY: 10, preventDefault: jest.fn() });
    expect(canvas.style.cursor).toBe('nwse-resize');

    selector.onMouseMove({ clientX: 50, clientY: 50, preventDefault: jest.fn() });
    expect(canvas.style.cursor).toBe('move');

    selector.onMouseMove({ clientX: 180, clientY: 180, preventDefault: jest.fn() });
  ...
  test('onMouseMove drags crop box within bounds', () => {
    ...
    selector.onMouseMove({ clientX: 60, clientY: 70, preventDefault: jest.fn() });
