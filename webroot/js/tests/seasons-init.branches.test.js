    ...
    if (sel === '#seasons-table' || sel === table) {
      return {
        ...
        DataTable: function (options) {
          ...
          return { destroy: () => {}, api: () => apiObj, _options: options };
        },
      };
    }
...
test('creates filter button and toggles panel when SearchBuilder present', async () => {
  ...
  window.$ = function (sel) {
    ...
    return {
      remove: () => {},
      empty: () => {},
      append: (el) => { const r = document.querySelector(sel); if (r && el) r.appendChild(el); },
      addClass: () => {},
    };
  };
...
test('SearchBuilder constructor throws -> placeholder shown', async () => {
  ...
  window.$ = function (sel) {
    ...
    return {
      remove: () => {},
      empty: () => {},
      append: (el) => { const r = document.querySelector(sel); if (r && el && el.nodeType) r.appendChild(el); },
      addClass: () => {},
    };
  };
