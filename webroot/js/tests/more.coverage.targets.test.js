...
describe('Additional coverage targets', () => {
    ...
    test('games_sport_dynamic renderEav creates cards for grouped fields', () => {
        ...
        const renderEav = gs.renderEav || (gs.__internals && gs.__internals.renderEav);
        ...
        expect(cards[0].querySelectorAll('input').length).toBeGreaterThanOrEqual(1);
    });
    ...
    `;
        ...
        if (saf && saf.SportAwareGameForm) {
            const inst = new saf.SportAwareGameForm({ selectId: 'team-season-select' });
    ...
    test('admin.renderAssociated handles associated objects with label/name', () => {
        ...
        expect(() =>
            internals.renderAssociated(document.getElementById('confirm-delete-modal'), associated)
        ).not.toThrow();
        const items = document.querySelectorAll('#confirm-delete-modal-assoc li');
