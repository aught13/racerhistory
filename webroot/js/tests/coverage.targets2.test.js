describe("More coverage targets to reach branch threshold", () => {
    ...
    test("admin uses source form and injects numeric parsed id and bulkAction", async () => {
        document.body.innerHTML = `
    ...
    `;
        ...
        const adminCjs = admin.__internals
            ? admin
            : admin.default ||
              (typeof require === "function"
                  ...
                  : admin);
        const internals =
            adminCjs.__internals ||
            (adminCjs.default && adminCjs.default.__internals) ||
            {};
    ...
    test("games_sport_dynamic.fetchMeta loads and renders template on success", async () => {
        ...
        const gs = await import("../../js/games_sport_dynamic");
        const fetchMeta =
    ...
    test("SportAwareGameForm shows fallback on fetch failure", async () => {
        ...
        const { default: Saf } = await import("../../js/sport-aware-game-form");
        const inst = new Saf();
