...
describe('More coverage targets to reach branch threshold', () => {
    ...
    test('games_sport_dynamic.fetchMeta loads and renders template on success', async () => {
        ...
        const fetchMeta = gs.fetchMeta || (gs.__internals && gs.__internals.fetchMeta);
        await fetchMeta('1');
        expect(document.getElementById('current-sport').textContent).toBe('Hockey');
        expect(
    ...
    test('SportAwareGameForm shows fallback on fetch failure', async () => {
        ...
        expect(document.getElementById('sport-specific-section').textContent).toContain(
            'Game Details'
        );
