...
describe('games_sport_dynamic helpers', () => {
    ...
    test('groupFields groups by field_group and defaults to general', () => {
        ...
        expect(Object.keys(grouped).sort()).toEqual(['one', 'two', 'general'].sort());
    ...
    test('buildFieldControl creates number input with min/max and text fallback', () => {
        const { buildFieldControl } = require('../../js/games_sport_dynamic.js');
        const numField = {
            field_name: 'score',
            display_label: 'Score',
            field_type: 'number',
        ...
        const txtField = { field_name: 'coach', display_label: 'Coach', field_type: 'text' };
    ...
    test('renderEav appends grouped cards to section', () => {
        ...
        const template = [
            { field_name: 'f1', display_label: 'F1', field_group: 'g1', field_type: 'text' },
            {
                field_name: 'f2',
                display_label: 'F2',
                field_group: 'g2',
    ...
    test('buildFieldControl number input without min/max has empty attrs', () => {
        const { buildFieldControl } = require('../../js/games_sport_dynamic.js');
        const nf = { field_name: 'n', display_label: 'N', field_type: 'number' };
