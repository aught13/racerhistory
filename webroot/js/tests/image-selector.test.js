...
describe('ImageSelector', () => {
  ...
  beforeEach(() => {
    ...
    window.Cropper = jest.fn().mockImplementation(() => ({
      ...
      getCroppedCanvas: jest.fn().mockReturnValue({
        toBlob: jest.fn((cb) => cb(new Blob(['test'], { type: 'image/jpeg' }))),
      }),
    }));
  ...
  describe('Constructor & Initialization', () => {
    ...
    test('should use custom aspect ratio if provided', () => {
      ...
      document.body.innerHTML += '<div id="custom-modal" class="modal fade"></div>';
    ...
    test('should handle missing modal gracefully', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Modal not found:', 'nonexistent-modal');
  ...
  describe('Image Loading', () => {
    test('should load images from /admin/images/browse', async () => {
      const mockImages = [
        { id: 1, thumbnail_url: '/thumb1.jpg', url: '/img1.jpg', tags: ['tag1'] },
        { id: 2, thumbnail_url: '/thumb2.jpg', url: '/img2.jpg', tags: ['tag2'] },
      ];
    ...
    test('should handle fetch errors gracefully', async () => {
      ...
      expect(imageSelector.gallery.innerHTML).toContain('Failed to load images');
    });
  ...
  describe('Gallery Rendering', () => {
    test('should render images correctly', () => {
      const images = [
        { id: 1, url: '/img1.jpg', thumbnail_url: '/thumb1.jpg', tags: ['tag1', 'tag2'] },
      ];
      imageSelector.renderGallery(images);

      const card = imageSelector.gallery.querySelector('[data-image-id="1"]');
    ...
    test('should handle empty image list', () => {
      ...
      expect(imageSelector.gallery.innerHTML).toContain('No images found');
    });
    ...
    test('should display image tags', () => {
      const images = [{ id: 1, url: '/img1.jpg', tags: ['tag1', 'tag2', 'tag3'] }];
      imageSelector.renderGallery(images);
  ...
  describe('Image Selection', () => {
    test('should select image on gallery card click', () => {
      ...
      const card = imageSelector.gallery.querySelector('[data-image-id="5"]');
      card.click();
  ...
  describe('Search & Filtering', () => {
    test('should filter images by ID', () => {
      ...
      const cards = imageSelector.gallery.querySelectorAll('[data-image-id]');
  ...
  describe('File Upload & Cropping', () => {
    test('should initialize cropper on file selection', () => {
      ...
      expect(imageSelector.cropImage.src).toBe('data:image/jpeg;base64,test');
  ...
  describe('Image Upload Process', () => {
    ...
    test('should set canvas dimensions based on aspect ratio', async () => {
      ...
      const call = imageSelector.cropper.getCroppedCanvas.mock.calls[0][0];
    ...
    test('should alert if no cropper when upload attempted', () => {
      imageSelector.cropper = null;
      const alertSpy = jest.spyOn(window, 'alert').mockImplementation();
