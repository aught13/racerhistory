...
describe('admin.js branch/error handling', () => {
    ...
    test('renderAssociated falls back on invalid JSON string and logs error', () => {
        ...
        const listItems = modal.querySelectorAll('#confirm-delete-modal-assoc li');
    ...
    test('delete handler catches parseInt error when parseInt throws', () => {
        ...
        admin.__internals.setContext({ deleteUrl: '/d', ids: '+42', idsName: 'ids[]' });
        ...
        expect(spyErr.mock.calls.some((c) => String(c[0]).includes('parseInt error'))).toBe(true);
    ...
    test('delete handler handles normalization/filter errors gracefully', () => {
        ...
        admin.__internals.setContext({ deleteUrl: '/pz', ids: badArr, idsName: 'ids[]' });

        document.getElementById('confirm-delete-modal-delete-btn').click();

        expect(spyErr).toHaveBeenCalled();
        expect(spyErr.mock.calls.some((c) => String(c[0]).includes('Error normalizing ids'))).toBe(
            true
        );
