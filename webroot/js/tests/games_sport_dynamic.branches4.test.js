...
describe('games_sport_dynamic branches', () => {
  ...
  test('buildFieldControl creates number input with min/max', () => {
    ...
    document.body.innerHTML = '<select id="team-season-select"></select><div id="sport-specific-section"></div><div id="sport-indicator"><span id="current-sport"></span></div><span id="sport-loading"></span>';
    const mod = require(path.resolve(__dirname, '../games_sport_dynamic.js'));
    const build = mod.buildFieldControl;
    const field = { field_name: 'pts', display_label: 'Points', field_type: 'number', min: 0, max: 99 };
  ...
  test('fetchMeta uses HTML fragment when available', async () => {
    ...
    global.fetch = jest.fn().mockResolvedValueOnce({ ok: true, text: async () => '<div data-sport-name="Footy"><p>frag</p></div>' });
  ...
  test('fetchMeta falls back to JSON when HTML fails', async () => {
    ...
    global.fetch = jest.fn()
      ...
      .mockResolvedValueOnce({ json: async () => ({ success: true, sportName: 'Hoops', eavTemplate: [], values: {} }) });

    await mod.fetchMeta('34');
    expect(document.getElementById('current-sport').textContent).toBe('Hoops');
  });
});
