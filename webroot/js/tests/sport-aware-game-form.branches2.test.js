...
describe('SportAwareGameForm branches', () => {
  ...
  test('handles non-ok fetch response by showing fallback', async () => {
    ...
    const SportAware = require(path.resolve(
      __dirname,
      '../sport-aware-game-form.js',
    ));
    ...
    expect(document.getElementById('sport-specific-section').innerHTML).toContain('Game Details');
    // current sport span should show an error message
    expect(document.getElementById('current-sport').textContent).toMatch(/Failed to load|Failed to load sport-specific/);
  });

  test('success with empty eavTemplate shows fallback', async () => {
    const json = { success: true, sportName: 'Soccer', eavTemplate: [] };
    global.fetch = jest.fn().mockResolvedValue({ ok: true, json: async () => json });
    const SportAware = require(path.resolve(__dirname, '../sport-aware-game-form.js'));
    const instance = new SportAware();
  ...
  test('renderSportFields creates number inputs and preserves existing values', () => {
    ...
    const data = {
      sportName: 'Hockey',
      eavTemplate: [
        { field_group: 'periods', field_name: 'period_1_mur', field_type: 'number', display_label: 'P1 Team', default_value: '5', min: 0, max: 20 },
        { field_group: 'periods', field_name: 'notes', field_type: 'text', display_label: 'Notes', default_value: 'x' },
      ],
    };
  ...
  test('updateSportFields maps eav values to legacy inputs', async () => {
    ...
    global.fetch = jest.fn().mockResolvedValue({ ok: true, json: async () => mapped });
