...
describe('SportAwareGameForm branch coverage', () => {
  ...
  test('maps EAV values to legacy inputs', async () => {
    const SportAwareGameForm = require(path.resolve(__dirname, '../sport-aware-game-form.js'));
    ...
    global.fetch = jest.fn().mockResolvedValue({
      ...
      json: async () => ({
        ...
        eavTemplate: { f1: { field_group: 'general', field_name: 'f1', display_label: 'F1' } },
  ...
  test('empty eavTemplate falls back to legacy fields', async () => {
    ...
    global.fetch = jest.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ success: true, sportName: 'Soccer', eavTemplate: [] }),
    });
  ...
  test('non-ok response shows error and falls back', async () => {
    ...
    expect(document.getElementById('current-sport').textContent).toMatch(/Failed to load/);
    expect(document.querySelector('[name="periods"]')).not.toBeNull();
  });
});
