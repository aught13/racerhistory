...
<div class="container py-4">
    <div class="row mb-3">
        <div class="col">
            ...
            <a href="<?= $this->Url->build(['prefix' => 'Admin', 'controller' => 'Persons', 'action' => 'add']) ?>" class="btn btn-success mb-3">
        ...
        <div class="col">
            ...
            <form id="bulk-action-form-persons" method="post">
                <div class="mb-2 d-flex align-items-center gap-2" id="persons-bulk-action-bar">
                    ...
                    <button type="submit" class="btn btn-primary btn-sm" id="bulk-action-btn-persons" disabled>Go</button>
                </div>
                ...
                <table class="table table-striped table-bordered" id="persons-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><input type="checkbox" name="person_ids[]" value="<?= $person->id ?>" class="person-checkbox"></td>
                            <td><?= h($person->display ?? ($person->first . ' ' . $person->last)) ?></td>
                            <td><?= h($person->first) ?></td>
                            <td><?= h($person->last) ?></td>
                            <td><?= h($person->birth) ?></td>
                            <td>
                                <a href="<?= $this->Url->build(['prefix' => 'Admin', 'controller' => 'Persons', 'action' => 'view', $person->id]) ?>" class="btn btn-sm btn-info">View</a>
                                <a href="<?= $this->Url->build(['prefix' => 'Admin', 'controller' => 'Persons', 'action' => 'edit', $person->id]) ?>" class="btn btn-sm btn-primary">Edit</a>
                                <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#confirm-delete-modal"
                                    ...
                                    data-item-type="person" data-form-id="<?= 'delete-form-person-' . $person->id ?>" aria-label="Delete person <?= h($person->display) ?>">Delete</button>
...
<script src="https://code.jquery.com/jquery-3.7.1.min.js" integrity="sha256-3gJwYp8p1H1mJk9g6r5Ge0XEt3G5UpiRaY1o1cnZ6+8=" crossorigin="anonymous"></script>
